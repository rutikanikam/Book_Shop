...
Class Master extends DBConnection {
	...
	function delete_product(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `products` where id = '{$id}' AND  status = '0'");
		if($del){
			$resp['status'] = 'success';
	...
	function delete_contact(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `tbl_contact` where contact_id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Contact successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function delete_subscription(){
		extract($_POST);
		$delete = $this->conn->query("DELETE FROM `tbl_newsletter` where id = '{$id}'");
		if($this->capture_err())
			return $this->capture_err();
		if($delete){
			$resp['status'] = 'success';
	...
	function add_to_cart(){
		...
		if($quantity > $available_quantity){
			return json_encode([
				...
				"message" => "Only {$available_quantity} items are available"
			]);
		}
		...
		if($check > 0){
			...
			$sql = "INSERT INTO `cart` (client_id, price, inventory_id, quantity)
					VALUES ({$this->settings->userdata('id')}, {$price}, {$inventory_id}, {$quantity} )";
		}
	...
	function place_order(){
		...
		if($success === true){
			...
			$data .= " ,amount = '{$amount}' ";
			$data .= " ,pscharge = '{$pscharge}' ";
			$data .= " ,paid = '{$paid}' ";
			$data .= " ,delivery_address = '{$delivery_address}' ";
			$data .= " ,o_contact = '{$o_contact}' ";
			$data .= " ,o_town = '{$o_town}' ";
			$data .= " ,o_pincode = '{$o_pincode}' ";
			$data .= " ,o_taluka = '{$o_taluka}' ";
			$data .= " ,o_district = '{$o_district}' ";
			...
			if($save_order){
				...
				while($row= $cart->fetch_assoc()):
					...
					$update_sql = "UPDATE inventory SET quantity=quantity-'{$row['quantity']}' where id='{$row['inventory_id']}'";
	...
	function save_book(){
		...
		$check = $this->conn->query("SELECT * FROM `upcoming_book` where `title` = '{$title}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
		...
		if(empty($id)){
			$sql = "INSERT INTO `upcoming_book` set {$data} ";
			$save = $this->conn->query($sql);
			$id= $this->conn->insert_id;
		}else{
			$sql = "UPDATE `upcoming_book` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			$upload_path = "uploads/book_1".$id;
			if(!is_dir(base_app.$upload_path))
				mkdir(base_app.$upload_path);
			if(isset($_FILES['img']) && count($_FILES['img']['tmp_name']) > 0){
				foreach($_FILES['img']['tmp_name'] as $k => $v){
	...
	function delete_book(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `upcoming_book` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Book successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_distributors(){
	  ...
	  $check = $this->conn->query("SELECT * FROM tbl_distributors where distributor_name = '{$distributor_name}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
	  if($this->capture_err())
	   return $this->capture_err();
	  if($check > 0){
	   $resp['status'] = 'failed';
	   $resp['msg'] = "Distributor already exist.";
	   return json_encode($resp);
	   exit;
	  }
	  if(empty($id)){
	   $sql = "INSERT INTO tbl_distributors set {$data}";
	   $save = $this->conn->query($sql);
	  }else{
	   $sql = "UPDATE tbl_distributors set {$data} where id = '{$id}' ";
	   $save = $this->conn->query($sql);
	  }
	  if($save){
	   $resp['status'] = 'success';
	   if(empty($id))
		$this->settings->set_flashdata('success',"New Distributor successfully saved.");
	   else
		$this->settings->set_flashdata('success',"Distributor successfully updated.");
	  }else{
	   $resp['status'] = 'failed';
	   $resp['err'] = $this->conn->error."[{$sql}]";
	  }
	 ...
	 function delete_distributor(){
	  extract($_POST);
	  $del = $this->conn->query("DELETE FROM tbl_distributors where id = '{$id}'");
	  if($del){
	   $resp['status'] = 'success';
	   $this->settings->set_flashdata('success',"Distributor successfully deleted.");
	  }else{
	   $resp['status'] = 'failed';
	   $resp['error'] = $this->conn->error;
	  }
...
switch ($action) {
	...
	case 'delete_subscription':
		echo $Master->delete_subscription();
	break;
	case 'delete_contact':
		echo $Master->delete_contact();
	break;
	case 'register':
	...
	case 'delete_img1':
		echo $Master->delete_img1();
	break;
	case 'save_distributors':
		echo $Master->save_distributors();
	break;
