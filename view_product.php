    ...
    <div class="container px-4 px-lg-5 my-5">
        ...
        <div class="row gx-4 gx-lg-5 align-items-center">
            ...
            <div class="col-md-6">
                ...
                <h4 class="fw-bolder border-bottom border-primary pb-1"><?php echo $title ?></h4>
                <p class="m-0"><small><b>By: </b><?php echo $author ?></small></p>
                <p class="m-0"><small><b>Language : </b><?php echo $language ?></small></p>
                <div class="fs-5 mb-5">
                <b>&#8377;</b> <span id="price"><?php echo number_format($inv[0]['price']) ?></span>
                <br>
                <span><small><b>Available Stock :</b> <span id="avail"><?php echo $inv[0]['quantity'] ?></span></small></span>
				<div id="availability_message" style="display:none; color:red;"></div>
                </div>
                ...
                <div class="d-flex">
					...
					<input type="hidden" name="available_quantity" value="<?php echo $inv[0]['quantity'] ?>">
                    <?php
					$query1 = "SELECT * FROM `inventory` WHERE product_id =  ".$id;                                    
					 ($fire1 = mysqli_query($conn, $query1)) or die("can not fetch the data from database." . mysqli_error($conn));						
					 if (mysqli_num_rows($fire1) > 0) 
					 {
						 while ($user1 = mysqli_fetch_assoc($fire1)) 
						 {						
						$book_qty =  $user1['quantity'];
						if($book_qty <= $book_qty && $book_qty != 0 ){
							?> 
							...
							?>
								<button class="btn btn-outline-dark flex-shrink-0 disabled"   type="submit"  style="background-color:#6f7580; color:#fff; " readonly align="center">OUT OF STOCK</button>
								<?php
										}
									}
								}
        ...
        $('#add-cart').submit(function(e){
            ...
            $.ajax({
                ...
                success:function(resp){
                    ...
                    }else if(resp.status == 'invalid_quantity'){
						$('#availability_message').html(resp.message);
