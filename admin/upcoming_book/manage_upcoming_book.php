...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT * from `upcoming_book` where id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
<div class="card card-outline card-info">
	...
	<div class="card-body">
		<form action="" id="book-form">
			...
			<div class="form-group">
                ...
                <textarea name="language" id="" cols="30" rows="2" class="form-control form no-resize"><?php echo isset($language) ? $language : ''; ?></textarea>
			</div>
            <div class="form-group">
	...
	<div class="card-footer">
		<button class="btn btn-flat btn-primary" form="book-form">Save</button>
...
<script>
    ...
    function delete_img1($path){
        start_loader()
        
        $.ajax({
            url: _base_url_+'classes/Master.php?f=delete_img1',
            data:{path:$path},
            method:'POST',
            dataType:"json",
            error:err=>{
	...
	$(document).ready(function(){
        $('.rem_img').click(function(){
            _conf("Are you sure to delete this image permanently?",'delete_img1',["'"+$(this).attr('data-path')+"'"])
        })
		...
		$('#book-form').submit(function(e){
			...
			$.ajax({
				...
				success:function(resp){
					if(typeof resp =='object' && resp.status == 'success'){
						location.href = "./?page=upcoming_book";
